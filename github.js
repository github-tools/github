...
(function() {
  ...
  var Github = function(options) {
    ...
    Github.User = function() {
      ...
      // -------+
      // username can be omitted or '' or null or undefined 
      ...
      this.userRepos = function(username, cb) {
        ...
        var command = username ? "/users/" + username : '/user';
        // Github does not always honor the 1000 limit so we want to iterate over the data set.
        _requestAllPages(command + "/repos?type=all&per_page=1000&sort=updated", function(err, res) {
          cb(err, res);
        });
      };
      ...
      this.userGists = function(username, cb) {
        ...
        var command = username ? "/users/" + username : '';
        _request("GET", command + "/gists", null, function(err, res) {
          cb(err,res);
        });
      };
    ...
    Github.Repository = function(options) {
      ...
      this.listBranches = function(cb) {
        ...
        _request("GET", repoPath + "/branches", null, function(err, branches) {
          if (err) return cb(err);
          cb(null, _.map(branches, function(branch){return branch.name}));
        });
      };
      ...
      // -------+ // TODO: when newTree is []

      this.remove = function(branch, path, cb) {
        updateTree(branch, function(err, latestCommit) {
          that.getTree(latestCommit+"?recursive=1", function(err, tree) {
            // Update Tree
      ...
      this.delete = function(branch, path, cb) {
        that.getSha(branch, path, function(err, sha) {
          ...
          _request("DELETE", delPath, params, cb);
        })
      }
    ...
    Github.Gist = function(options) {
      ...
      this.fork = function(cb) {
        _request("POST", gistPath+"/forks", null, function(err,res) {
          cb(err,res);
        });
      };
    ...
    this.getRepo = function(user, repo) {
      return new Github.Repository({user: user, repo: repo});
    };
